...
public class DKIMSigner {
	...
	private SigningAlgorithm signingAlgorithm = SigningAlgorithm.SHA256withRSA; // use
																				// rsa-sha256
																				// by
																				// default,
																				// see
	...
	public DKIMSigner(String signingDomain, String selector, byte[] raw) throws Exception {
		...
		// decode private key
		PKCS8EncodedKeySpec privSpec = new PKCS8EncodedKeySpec(raw);
	...
	public void setIdentity(String identity) throws DKIMSignerException {
		...
		if (identity != null) {
			...
			if (!identity.endsWith("@" + this.signingDomain) && !identity.endsWith("." + this.signingDomain)) {
				throw new DKIMSignerException("The domain part of " + identity + " has to be " + this.signingDomain
						+ " or its subdomain");
			}
		}
	}
	...
	public void setSigningAlgorithm(SigningAlgorithm signingAlgorithm) throws DKIMSignerException {
		...
		try {
			...
			throw new DKIMSignerException("The hashing algorithm " + signingAlgorithm.getJavaHashNotation()
		...
		try {
			...
			throw new DKIMSignerException("The signing algorithm " + signingAlgorithm.getJavaSecNotation()
					+ " is not known by the JVM", nsae);
		}
		...
		synchronized (this) {
			...
			try {
				this.signatureService.update(headerContent
						.append(this.headerCanonicalization.canonicalizeHeader(DKIMSIGNATUREHEADER, " "
